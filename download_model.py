...
def download_huggingface(model_id: str, save_dir: str):
    ...
    print(f"Downloading {model_id} from HuggingFace -> {save_dir}")
...
def download_modelscope(model_id: str, save_dir: str):
    ...
    try:
        ...
        print(f"Downloading {model_id} from ModelScope -> {save_dir}")
        ms_dl(model_id, local_dir=save_dir)
        print(f"Done: {save_dir}")
    except ImportError:
        print("ERROR: modelscope tidak terinstall. Jalankan: pip install modelscope>=1.15.0")
        raise
...
if __name__ == "__main__":
    ...
    print("Jalankan server: python server.py")
